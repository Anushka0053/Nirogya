...
export class APIService {
  ...
  static async analyzeMedicalImage(_file: File, imageType: 'xray' | 'wound'): Promise<Record<string, unknown>> {
    try {
      // Simulate API delay
  ...
  private static getMockImageAnalysis(imageType: 'xray' | 'wound'): Record<string, unknown> {
    if (imageType === 'xray') {
      const xrayResults = [
        {
