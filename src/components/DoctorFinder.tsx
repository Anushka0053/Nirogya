...
import { MapPin, Star, Clock, Phone, Navigation, Calendar, User, Loader, CheckCircle, Video, UserCheck, AlertTriangle, RefreshCw, Award, GraduationCap, Languages, DollarSign, MapPinIcon, Mail, Stethoscope } from 'lucide-react';
...
const DoctorFinder: React.FC = () => {
  ...
  const searchDoctors = async () => {
    ...
    try {
      const filtered = getFilteredDoctors(selectedSpecialization, location || undefined);
      setDoctors(filtered);
    } catch {
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const appointmentData = {
        ...
        notes: `${bookingData.notes ? bookingData.notes + ' | ' : ''}Appointment Type: ${selectedType?.name} | Fee: ₹${bookingDoctor.consultationFee[selectedAppointmentType === 'in-person' ? 'inPerson' : selectedAppointmentType]}`
      };
                ...
                <>
                  ...
                  <form onSubmit={handleBookingSubmit} className="space-y-4 sm:space-y-6">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
                        {appointmentTypes.map((type) => {
                          ...
                          const fee = bookingDoctor.consultationFee[type.id === 'in-person' ? 'inPerson' : type.id];
                          return (
                            <label
                              key={type.id}
                            ...
                            >
                              <input
                                ...
                                onChange={(e) => setSelectedAppointmentType(e.target.value as 'in-person' | 'call' | 'video')}
                                className="sr-only"
                              />
                              <div className="text-center">
                    ...
                    <div className="bg-purple-500/20 border border-purple-500/30 rounded-lg p-4 sm:p-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-purple-200">
                        <div className="space-y-1">
                          ...
                          <p><strong>Fee:</strong> ₹{bookingDoctor.consultationFee[selectedAppointmentType === 'in-person' ? 'inPerson' : selectedAppointmentType]}</p>
                        </div>
                        <div className="space-y-1">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4">
                      ...
                      >
                        Confirm Booking (₹{bookingDoctor.consultationFee[selectedAppointmentType === 'in-person' ? 'inPerson' : selectedAppointmentType]})
                      </button>
                    </div>
                  </form>
                </>
